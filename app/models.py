...
class Component(db.Model):
    ...
    def query_all_with_active_incidents():
        ...
        """
        subq = (
            db.session.query(Incident, IncidentComponentRelation)
            .join(
            ...
            .distinct(IncidentComponentRelation.component_id)
        ).subquery()
        incident_subq = aliased(Incident, subq, name="incident")
        incident_comp_subq = aliased(IncidentComponentRelation, subq,
                                     name="incident_comp")
#        return (
...
#        )
        ...
        return (
            db.session.query(Component, incident_subq)
            .join(
                subq,
                and_(
                    incident_comp_subq.component_id == Component.id,
                ),
                isouter=True,
            )
            #.join(
